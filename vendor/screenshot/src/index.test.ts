...
/**
 * Bounds of the dark ink in a column range, or null where there is none.
 *
 * Lets a test ask where painted text starts and stops without depending on the
 * font's own metrics: the shape of a run of ink says more than its position.
 */
function darkInkBounds(
  canvas: HTMLCanvasElement,
  fromX: number,
  toX: number
): { top: number; bottom: number; left: number; right: number } | null {
  const context = canvas.getContext('2d')!
  const left = Math.max(0, Math.min(fromX, canvas.width - 1))
  const right = Math.max(left, Math.min(toX, canvas.width - 1))
  const width = right - left + 1
  const { data } = context.getImageData(left, 0, width, canvas.height)
  ...
  for (let index = 0; index < data.length; index += 4) {
    const isInk = data[index + 3] > 64 && data[index] < 128
    if (!isInk) continue
    const pixel = index / 4
    const x = left + (pixel % width)
  ...
  if (inkRight < 0) return null

  return { top, bottom, left: inkLeft, right: inkRight }
}
...
describe('screenshot', () => {
  ...
  describe('visual accuracy: border radius clipping', () => {
    it('keeps a child that overflows a rounded parent with visible overflow', async () => {
      const outer = createElement({
        width: '120px',
        height: '40px',
        position: 'relative',
      ...
      // A rounded track with a knob standing proud of it, which is what a
      // slider or a progress bar looks like. CSS does not clip the knob,
      // because the track's own overflow is visible.
      ...
      // Above the track, where only the knob reaches.
      expectColor(sampleArea(canvas, 58, 12, 3), { r: 0, g: 0, b: 255, a: 255 }, 10)
      // Over the track itself.
      expectColor(sampleArea(canvas, 58, 19, 3), { r: 0, g: 0, b: 255, a: 255 }, 10)
      // Beside the track, which nothing paints.
      expectColor(sampleArea(canvas, 3, 19, 3), { r: 255, g: 255, b: 255, a: 255 }, 10)
    })

    it('clips a child to a rounded parent that hides its overflow', async () => {
      const outer = createElement({
        width: '120px',
        height: '40px',
        position: 'relative',
      ...
      expectColor(sampleArea(canvas, 58, 12, 3), { r: 255, g: 255, b: 255, a: 255 }, 10)
      expectColor(sampleArea(canvas, 58, 19, 3), { r: 0, g: 0, b: 255, a: 255 }, 10)
    })

    it('clips the element own background to its border radius', async () => {
      ...
      const canvas = await screenshot.canvas(circle, {
        scale: 1,
        backgroundColor: null,
      })

      // The corner is outside the circle, so nothing was painted there.
      expect(getPixel(canvas, 1, 1).a).toBe(0)
      expectColor(sampleArea(canvas, 18, 18, 4), { r: 255, g: 0, b: 0, a: 255 }, 10)
    })
  })
  ...
  describe('visual accuracy: transformed SVG', () => {
    it('rotates an inline svg with its CSS transform', async () => {
      const wrapper = createElement({
        width: '60px',
        height: '60px',
        position: 'relative',
      ...
      // A band along the top of the icon. Rotating the icon a quarter turn
      // about its centre puts the band down its right-hand edge, so where the
      // band ends up says whether the transform was honoured or dropped.
      wrapper.innerHTML = `
        <svg width="20" height="20" viewBox="0 0 20 20"
          style="position:absolute;left:20px;top:20px;transform:rotate(90deg);transform-origin:center">
          <rect x="0" y="0" width="20" height="6" fill="rgb(255, 0, 0)" />
        </svg>
      `
      container.appendChild(wrapper)
      ...
      // Down the right-hand edge of the icon, where the rotation put the band.
      expectColor(sampleArea(canvas, 36, 29, 3), { r: 255, g: 0, b: 0, a: 255 }, 12)
      // Along the top, where the band would still be had the rotation been lost.
      expectColor(
        sampleArea(canvas, 28, 21, 3),
        { r: 255, g: 255, b: 255, a: 255 },
        12
      )
    })

    it('keeps an svg translated by a transform in place', async () => {
      const wrapper = createElement({
        width: '60px',
        height: '60px',
        position: 'relative',
      ...
      // The pattern used to centre an icon in a form control, which the
      // renderer has to keep resolving through the layout rect.
      wrapper.innerHTML = `
        <svg width="20" height="20" viewBox="0 0 20 20"
          style="position:absolute;left:10px;top:0;transform:translateY(30px)">
      ...
      expectColor(sampleArea(canvas, 18, 38, 4), { r: 0, g: 0, b: 255, a: 255 }, 12)
      expectColor(
        sampleArea(canvas, 18, 10, 4),
        { r: 255, g: 255, b: 255, a: 255 },
        12
      )
    })
...
<aside id="category" style="-webkit-text-size-adjust: 100%;
  ...
  describe('visual accuracy: text overflow', () => {
    const truncating = {
      width: '100px',
      height: '30px',
    ...
    it('ends truncated text with an ellipsis', async () => {
      const element = createElement({
        ...truncating,
        textOverflow: 'ellipsis',
      })
      element.textContent = 'HHHHHHHHHHHHHHHH'
      container.appendChild(element)

      const canvas = await screenshot.canvas(element, { scale: 1 })

      const all = darkInkBounds(canvas, 0, canvas.width - 1)
      expect(all).not.toBeNull()
      const tail = darkInkBounds(canvas, all!.right - 8, all!.right)
      expect(tail).not.toBeNull()
      // The run ends in dots sitting on the baseline rather than in capitals,
      // so its last few columns start well below the top of the text.
      expect(tail!.top).toBeGreaterThan(all!.top + 5)
      // And the ellipsis fits inside the box instead of being cut off by it.
      expect(all!.right).toBeLessThan(canvas.width - 2)
    })

    it('cuts text off at the box when overflow is clipped', async () => {
      const element = createElement({
        ...truncating,
        textOverflow: 'clip',
      })
      ...
      // Letters run all the way to the edge, where they are simply cut.
      expect(tail!.top).toBeLessThan(all!.top + 3)
      expect(all!.right).toBeGreaterThan(canvas.width - 4)
    })
  })

  describe('visual accuracy: range input', () => {
    /**
     * A slider on a dark card, laid out so the test can name coordinates: the
     * track runs from x=20 to x=100 with its centre on y=12, and the thumb's
     * centre travels between x=28 and x=92.
     */
    const mountSlider = (accentColor: string, value: number) => {
      const wrapper = createElement({
        width: '120px',
        height: '24px',
        padding: '10px 20px',
    ...
    it('darkens the unfilled track when the accent is too pale to show on a light one', async () => {
      const wrapper = mountSlider('rgb(255, 255, 255)', 70)
      const canvas = await screenshot.canvas(wrapper, { scale: 1 })

      // Filled, then the remainder past the thumb, which has to be a grey the
      // white fill can be told apart from rather than a near-white one.
      expectColor(
        sampleArea(canvas, 24, 12, 2),
        { r: 255, g: 255, b: 255, a: 255 },
        12
      )
      expectColor(sampleArea(canvas, 96, 12, 2), { r: 59, g: 59, b: 59, a: 255 }, 24)
    })

    it('fills the track up to the thumb in the accent colour', async () => {
      const wrapper = mountSlider('rgb(255, 0, 0)', 70)
      const canvas = await screenshot.canvas(wrapper, { scale: 1 })
      ...
      // The thumb, standing proud of the track and painted in the accent too.
      expectColor(sampleArea(canvas, 73, 7, 2), { r: 255, g: 0, b: 0, a: 255 }, 12)
      // Past it the track is the light grey a dark accent shows against, and
      // nothing is painted above the track there.
      expectColor(
        sampleArea(canvas, 96, 12, 2),
        { r: 239, g: 239, b: 239, a: 255 },
        24
      )
      expectColor(
        sampleArea(canvas, 96, 7, 2),
        { r: 10, g: 10, b: 10, a: 255 },
        12
      )
    })

    it('keeps the thumb within the track at either end of its travel', async () => {
      const empty = await screenshot.canvas(mountSlider('rgb(255, 0, 0)', 0), {
        scale: 1,
      })
      const full = await screenshot.canvas(mountSlider('rgb(255, 0, 0)', 100), {
        scale: 1,
      })

      // At zero the thumb sits over the left end of the track rather than
      // half outside it, and there is no fill to its left.
      expectColor(sampleArea(empty, 28, 7, 2), { r: 255, g: 0, b: 0, a: 255 }, 12)
      expectColor(sampleArea(empty, 18, 7, 2), { r: 10, g: 10, b: 10, a: 255 }, 12)
      expectColor(
        sampleArea(empty, 60, 12, 2),
        { r: 239, g: 239, b: 239, a: 255 },
        24
      )

      // At full it sits over the right end, with the whole track filled.
      expectColor(sampleArea(full, 92, 7, 2), { r: 255, g: 0, b: 0, a: 255 }, 12)
      expectColor(sampleArea(full, 60, 12, 2), { r: 255, g: 0, b: 0, a: 255 }, 12)
    })
  })

  describe('visual accuracy: text shadow', () => {
    it('casts a single text shadow on single-line text', async () => {
      const element = createElement({
        width: '160px',
        height: '60px',
        fontFamily: 'system-ui, sans-serif',
        ...
        color: 'rgb(0, 0, 0)',
        backgroundColor: 'rgb(255, 255, 255)',
        // Offset well clear of the glyphs and unblurred, so the shadow is
        // countable rather than a smudge under the text.
        textShadow: '0 24px 0 rgb(255, 0, 0)',
      })
      element.textContent = 'Shadow'
      container.appendChild(element)

      const canvas = await screenshot.canvas(element, { scale: 1 })
      const context = canvas.getContext('2d')!

      const countRed = (top: number, height: number): number => {
      ...
      // The shadow lands a line and a half below the text it belongs to.
      expect(countRed(26, 24)).toBeGreaterThan(20)
      // And the text itself is still painted where it was, in its own colour.
      expect(countRed(0, 24)).toBe(0)
    })

    it('spreads a blurred shadow as far as its blur radius', async () => {
      const element = createElement({
        width: '200px',
        ...
        color: 'rgb(0, 0, 0)',
        backgroundColor: 'rgb(255, 255, 255)',
        textShadow: '0 0 12px rgb(255, 0, 0)',
      })
      element.textContent = 'Glow'
      container.appendChild(element)

      const canvas = await screenshot.canvas(element, { scale: 1 })
      const context = canvas.getContext('2d')!

      const countTinted = (top: number, height: number): number => {
        const { data } = context.getImageData(0, top, canvas.width, height)
        let count = 0
        for (let index = 0; index < data.length; index += 4) {
          if (data[index] - data[index + 1] > 20) count += 1
        }
        return count
      }

      const ink = darkInkBounds(canvas, 0, canvas.width - 1)
      expect(ink).not.toBeNull()

      // A 12px blur reaches several pixels clear of the glyphs it came from.
      expect(countTinted(ink!.top - 9, 5)).toBeGreaterThan(10)
      // And no further than that: the padding above stays white.
      expect(countTinted(0, 5)).toBe(0)
    })
  })
