...
`.perf/fidelity.mjs` pixel-matches the gallery against a stashed pre-change
run, `npm run snapdom:test` runs the vendored suites nearest these files in
real Chromium, and `.perf/live.mjs` reports what each mirror on the live page
actually receives.
...
- **The base reset CSS is factored** (`generateDedupedBaseCSS`, PERF-6). Tag
  ...
  overrides after it. 33kb of the player card's SVG became 9kb, and the SVG is
  re-parsed by every rasterization, so bytes are decode time.
...
- **The cost being optimized was half wait.** A capture was timed across the
  ...
  main-thread time only (`CaptureHandle` in `src/lib/mirror-engines.ts`) and
  overlaps the decode with the next frame.
...
`vitest.snapdom.config.ts` at the repo root:

```bash
npm run snapdom:test
```
...
checked with `.perf/fidelity.mjs` against the gallery (diffed against a
stashed baseline run) and `.perf/snapdom.mjs` for cost and freshness.
